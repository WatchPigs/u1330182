...
namespace Engine
{
	namespace JobSystem
	{
		...
		{
			std::map<HashedString, JobQueueData*>::iterator existing = Queues.find(i_JobData.QueueName);
			assert(existing != Queues.end());
			assert(existing->second);

			DEBUG_PRINT("Job System: Adding Job to Queue \"%s\".", existing->second->m_SharedQueue.GetName().c_str());
			existing->second->m_SharedQueue.Add(new QueuedJob(i_JobData, i_JobName, i_JobFunction));
		}

		JobData GetCurrentThreadActiveJobData()
		{
			return SharedJobQueue::GetCurrentThreadActiveJobData();
		}
